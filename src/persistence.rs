...
impl PersistenceManager {
    ...
    pub fn load_config(&self) -> Result<Config> {
        ...
        validated_config.bookmarks_panel_width = validated_config.bookmarks_panel_width.clamp(20, 80);
    ...
    pub fn load_reading_progress(&self) -> Result<HashMap<String, ReadingProgress>> {
        ...
        let content = fs::read_to_string(&progress_path)
            .context("Failed to read reading progress file")?;
        
        let progress: HashMap<String, ReadingProgress> = serde_json::from_str(&content)
            .unwrap_or_else(|e| {
                tracing::warn!("Failed to parse reading progress file: {}. Starting fresh.", e);
                HashMap::new()
            });
    ...
    pub fn save_reading_progress(&self, progress: &HashMap<String, ReadingProgress>) -> Result<()> {
        ...
        fs::write(&progress_path, content)
            .context("Failed to write reading progress file")?;
    ...
    pub fn load_recent_books(&self) -> Result<Vec<String>> {
        ...
        let content = fs::read_to_string(&recent_path)
            .context("Failed to read recent books file")?;
        
        let books: Vec<String> = serde_json::from_str(&content)
            .unwrap_or_else(|e| {
        ...
        let existing_books: Vec<String> = books.into_iter()
    ...
    pub fn save_recent_books(&self, books: &[String]) -> Result<()> {
        ...
        let content = serde_json::to_string_pretty(books)
            .context("Failed to serialize recent books")?;
        
        fs::write(&recent_path, content)
            .context("Failed to write recent books file")?;
    ...
    pub fn load_bookmarks(&self, book_path: &str) -> Result<Vec<Bookmark>> {
        ...
        let file: BookmarksFile = serde_json::from_str(&content)
            .unwrap_or_else(|e| {
                ...
                BookmarksFile { bookmarks: Vec::new() }
            });
